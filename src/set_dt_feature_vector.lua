...
local function set_dt_feature_vector(str_feature_vector, feature_vector, g_n_dt_feature_vector)
	local feature_table = json.decode(str_feature_vector)
	-- does the conversions
	feature_table = generate_features(feature_table)
	local ohe = one_hot_encoding(feature_table)
	local n_features = get_num_features()
	assertx(g_n_dt_feature_vector == n_features, "C's provided g_n_dt_feature_vector value of ", g_n_dt_feature_vector, ", does not match with Lua's value of ", n_features)
	-- for debugging
	for k, v in pairs(ohe) do print(k, v) end
	-- end debugging
