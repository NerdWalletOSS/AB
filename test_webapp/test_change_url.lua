...
tests.t2 = function (
  ...
  local description = "When state == started and OverWrite = false, then no change in URL possible"
  if ( just_pr ) then print(description) return end
 -- START: Make some test 
  reset_db()
  local tid = assert(mk_rand_test({TestType = "XYTest"}))
...
tests.t3 = function (
  ...
  local description = "When state == started and OverWrite = true and new URLs are same as old then no new rows created in Variant table"
  if ( just_pr ) then print(description) return end
 -- START: Make some test 
  reset_db()
  local tid = assert(mk_rand_test({TestType = "XYTest"}))
...
tests.t4 = function (
  ...
  local description = "When state != started and OverWrite = true, then no new rows created in Variant table"
  -- START: Make some test 
  reset_db()
  local tid = assert(mk_rand_test({TestType = "XYTest"}))
  local T1 = get_test_info(tid)
